...
os.makedirs('HW_4/HW4_output', exist_ok=True)

#quarterly factors constructed in S3 
demo_factors=pd.read_csv('HW_4/demo_factors.csv')
...
common=pd.read_csv('Python data/common.csv')
...
#save to csv
mom_perf.to_csv('./HW_4/HW4_output/mom_perf.csv', index=False)



#%%
#Form portfolios using three factors (e.g., momentum, size, and illiquidity)

# ------------------------------------------------------------------
# 2. Form portfolios based on size factor*
# ------------------------------------------------------------------
...
#save file
perf_sizemom.to_csv('./HW_4/HW4_output/perf_sizemom.csv', index=False)
...
# ------------------------------------------------------------------
# 3. Form portfolios based on illiquidity factor*
# ------------------------------------------------------------------

three_factors=demo_factors.copy()

# Adjust the sign of the variables so that they have positive relation with stock returns.
three_factors['ILLIQ'] = -1 * three_factors['ILLIQ'] #the illiquidity factor needs to be changed
three_factors['SIZE'] = -1 * three_factors['SIZE'] #the size factor needs to be changed
three_factors['PRRET'] = 1 * three_factors['PRRET'] #the momentum factor does not need to be change

# Rank stocks by each sign-adjusted signal into percentiles
three_factors = three_factors.dropna(subset=['ILLIQ', 'SIZE', 'PRRET'])
three_factors['ILLIQ_RANK'] = three_factors.groupby('QNO')['ILLIQ'].transform(lambda x: pd.qcut(x, q=100, labels=False))
three_factors['SIZE_RANK'] = three_factors.groupby('QNO')['SIZE'].transform(lambda x: pd.qcut(x, q=100, labels=False))
three_factors['MOM_RANK'] = three_factors.groupby('QNO')['PRRET'].transform(lambda x: pd.qcut(x, q=100, labels=False))

# Take the average of the above percentile ranks across variables in the same category, i.e., “category combo"
three_factors['ILLIQ_CAT'] = three_factors['ILLIQ_RANK']
three_factors['SIZE_CAT'] = three_factors['SIZE_RANK']
three_factors['MOM_CAT'] = three_factors['MOM_RANK']

# Also take the average across the all category combos to form an “overall combo” variable, and form decile portfolios based on the overall combo
three_factors['OVERALL_COMBO'] = (three_factors['ILLIQ_RANK'] + three_factors['SIZE_RANK'] + three_factors['MOM_RANK']) / 3
three_factors['OVERALL_DECILE'] = three_factors.groupby('QNO')['OVERALL_COMBO'].transform(lambda x: pd.qcut(x, q=10, labels=False))

# Combine with future 1 quarter return
three_factors.sort_values(by=['PERMNO', 'QNO'], inplace=True)
three_perf = pd.merge(three_factors, fret, on=['PERMNO','QNO'], how='inner')

three_perf.to_csv('./HW_4/HW4_output/perf_illiquidity.csv', index=False)
