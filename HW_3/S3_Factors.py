# -*- coding: utf-8 -*-
"""
Created on Wed Nov 22 17:03:10 2023
...
dsf=dsf[['PERMNO','DATE','QNO','ILQ','TURND','RET']]
...
illiq = dsf.groupby(['PERMNO','QNO']).agg(mea_TURND=('TURND', 'mean'), mea_ILLIQ=('ILQ', 'mean'), n_turn=('TURND', 'count'), n_ilq=('ILQ', 'count')).reset_index()
...
illiq=illiq[['PERMNO','QNO','mea_ILLIQ','mea_TURND']]
...
#quarterly returns, illiquidity ratio and average daily turnover
qmout.sort_values(by=['PERMNO','QNO'], inplace=True)
illiq.sort_values(by=['PERMNO','QNO'], inplace=True)
size1.sort_values(by=['PERMNO','QNO'], inplace=True)

qmout1=pd.merge(qmout, illiq, on=['PERMNO', 'QNO'], how='left')
qmout1=pd.merge(qmout1, size1, on=['PERMNO', 'QNO'], how='left')
qmout1=qmout1[['PERMNO','QNO','QRET','TURND','ILLIQ','SIZE','PRC']]
...
demo_factors=demo_factors[['PERMNO','QNO','PRRET','QRET','TURND','ILLIQ','ex','SIZE','PRC']]
